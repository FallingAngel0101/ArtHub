...
export default function ArtworkDetail() {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      alert(
        `Покупка успешна! ID документа: ${result.document_id}\nИтоговая цена: ${result.final_price.toLocaleString()} ₽`,
      );
      navigate("/catalog");
    } catch {
      alert(
        "Ошибка покупки. Возможно, картина уже продана или вы не коллекционер.",
      );
    } finally {
      setBuyLoading(false);
    }
  ...
  const handleGenerateContract = async () => {
    if (!artwork) return;
    try {
      const res = await api.post(`/documents/sale/${artwork.id}`);
  ...
  if (!artwork)
    return <div className="container error">Произведение не найдено</div>;
      ...
      <div className="detail-header">
        ...
        <div className="detail-info">
          ...
          {userRole === "collector" && artwork.status === "published" && (
            <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
              ...
              >
                {buyLoading
                  ? "Покупка..."
                  : `Купить за ${artwork.current_price.toLocaleString()} ₽`}
              </button>
          ...
          <div className="detail-meta">
            <p>
              <strong>Техника:</strong> {artwork.technique}
            </p>
            <p>
              <strong>Год:</strong> {artwork.year}
            </p>
            <p>
              <strong>Размеры:</strong> {artwork.width} × {artwork.height} см
            </p>
            <p>
              <strong>Базовая цена:</strong>{" "}
              {artwork.base_price.toLocaleString()} ₽
            </p>
            <p>
